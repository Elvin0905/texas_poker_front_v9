...
const DEAL_CARD_TARGET_OFFSET_Y = -25;
// 主玩家（hero）手牌位置獨立微調
...
const SEAT_BET_AMOUNT_POSITIONS_6 = [
  ...
  { x: 515, y: 430 },  // 座位 2（右上角：下注籌碼左移 15px；跟著座位整體再左移 10px）
  { x: 330, y: 215 },  // 座位 3（下注籌碼：上移 100px、左移 10px）
  { x: 240, y: 295 },  // 座位 4（左上角：下注籌碼移到手牌右側、下移 10px）
...
const SEAT_POSITIONS_6 = [
  ...
  { x: 625, y: 505 }, // 右上角（再下移 50px、右移 30px；整體左移 10px）
  { x: 430, y: 215 },
  { x: 115, y: 295 },
  { x: 80, y: 575 },
];
    ...
    positions.forEach((pos, idx) => {
      ...
      const holeCards = Array.from({ length: DEAL_CARD_MAX_HOLE_COUNT }, (_, cardIndex) => {
        ...
        const sprite = this.add
          ...
          .setDepth(SEAT_HOLE_CARD_DEPTH + (avatarFlipX ? (1 - cardIndex) : cardIndex) * 0.1)
          .setAngle(angle)
          .setVisible(false);
        const cardMaskGfx = null;
        const cardShadow = this.add.graphics()
          .setDepth(SEAT_HOLE_CARD_DEPTH + (avatarFlipX ? (1 - cardIndex) : cardIndex) * 0.1 - 0.5)
          .setVisible(false);
...
// Keep text anchored to avatar when switching hero/non-hero layout.
  updateSeatTextLayout(seatView, isHero) {
    ...
    seatView.holeCards?.forEach((hc, i) => {
      ...
      hc.cardShadow?.setDepth(holeCardDepth + flipStep * 0.1 - 0.5);
    });
  ...
  updateSeatRoleBadgeLayout(seatView) {
    ...
    const badgeY = seatView.posY + ROLE_BADGE_Y_ABOVE_HEAD - (isHeroSeat ? 15 : 0);
  ...
  resolveDealTargetPosition(seatView, cardIndexRaw) {
    ...
    const heroNudgeX = isHeroNudge ? 5 : 0;
    return {
    ...
    if (table) {
      ...
      for (let index = 0; index < this.seatViews.length; index += 1) {
        ...
        if (!player) {
          ...
          // 只有觀戰者能點空位入座（take_seat）；伺服器僅接受觀戰桌的 take_seat。
          if (this.state?.isSpectator) {
        ...
        // CJK names appear visually larger at same px — subtract 2px from final size
        if (/[一-鿿㐀-䶿]/.test(seatView.name.text)) {
          seatView.name.setFontSize(`${Math.max(12, parseInt(seatView.name.style.fontSize, 10) - 4)}px`);
        }
