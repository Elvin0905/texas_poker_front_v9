...
export class RegisterScene extends Phaser.Scene {
  ...
  create() {
    ...
    this._syncBound = () => {
      ...
      if (a === this._emailEl || a === this._nickEl || a === this._pwEl || a === this._confirmPwEl) {
    ...
    this._onInputBlur = () => {
      ...
      this._kbTimer = setTimeout(() => {
        const a = document.activeElement;
        const inputs = [this._emailEl, this._nickEl, this._pwEl, this._confirmPwEl];
  ...
  _buildScene() {
    ...
    this.confirmEyeIcon.on("pointerup", () => this._toggleConfirmPasswordVisible());
  ...
  _syncInputPositions() {
    ...
    [this._emailEl, this._nickEl, this._pwEl, this._confirmPwEl].forEach(el => el.style.removeProperty("display"));
  ...
  _showVerifyModal(type = "email") {
    ...
    applyGoldTitleGradient(this._vmTitle);
    this._vmDescText.setText(descStr).setPosition(cx, t + 130).setVisible(true);
