...
export class LobbyScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.settingsTitleText = this.add
      .text(layout.centerX, layout.centerY, "設定", {
        fontFamily: "sans-serif",
        fontSize: "36px",
        color: "#ecd5b5",
        fontStyle: "bold",
    ...
    // Initialize to current value so a stale pendingOpenDailySettlement from a
    // previous replay doesn't re-open the modal on unrelated lobby recreations.
    this._lastPendingDailySettlementVersion = this.store.getState?.().pendingOpenDailySettlement ?? 0;
  ...
  renderState(state) {
    const pdv = state.pendingOpenDailySettlement ?? 0;
    if (pdv > this._lastPendingDailySettlementVersion) {
      this._lastPendingDailySettlementVersion = pdv;
      this.openHandReportsModal();
    }
  ...
  applyLayout() {
    ...
    this.settingsTitleText?.setPosition(layout.centerX, layout.centerY);
