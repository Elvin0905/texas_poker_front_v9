...
const HAND_RESULT_LIST_START_Y = 380;
const HAND_RESULT_ROW_GAP = 136;
...
const HAND_RESULT_AMOUNT_X_OFFSET = -286;
const HAND_RESULT_RANK_X_OFFSET = -156;
const HAND_RESULT_CARDS_START_X_OFFSET = -20;
const HAND_RESULT_NAME_Y_OFFSET = -36;
const HAND_RESULT_DETAIL_Y_OFFSET = 24;
const HAND_RESULT_CARDS_Y_OFFSET = 2;
const HAND_RESULT_CARD_WIDTH = 60;
const HAND_RESULT_CARD_HEIGHT = 84;
...
// 主玩家（hero）手牌位置獨立微調
const HERO_DEAL_CARD_TARGET_OFFSET_X_LEFT = 430;        // 手機版：靠右
const HERO_DEAL_CARD_TARGET_OFFSET_X_RIGHT = 510;       // 手機版：靠右
const HERO_DEAL_CARD_TARGET_OFFSET_X_LEFT_DESKTOP = 430;  // 桌機版：完全靠右
const HERO_DEAL_CARD_TARGET_OFFSET_X_RIGHT_DESKTOP = 510; // 桌機版：完全靠右
...
export class TableScene extends Phaser.Scene {
  ...
  buildHandResultDisplayRows(handResult) {
    ...
    const rows = rawRows.map((item, index) => {
      ...
      const username = String(item?.username || `玩家${index + 1}`);
      ...
      const isFold = !rankText && cardFrames.length <= 0 && (!Number.isFinite(winAmount) || winAmount <= 0);
      const amountText = formatSignedAmount(netAmount);
  ...
  openHandResultModal(handResult) {
    ...
    const _hrTitleLabelCenterY = HAND_RESULT_PANEL_Y - HAND_RESULT_PANEL_HEIGHT / 2;
    const _labelBottom = _hrTitleLabelCenterY + 56; // half of 112px display height
    const _dynamicListStartY = _labelBottom + 20 + HAND_RESULT_ROW_HEIGHT / 2 + (this.modalDy || 0);
    rows.forEach((row, index) => {
      const rowY = _dynamicListStartY + index * HAND_RESULT_ROW_GAP;
      ...
      const amountText = this.add
        .text(CENTER_X + HAND_RESULT_AMOUNT_X_OFFSET, detailY, row.amountText, {
          fontSize: HAND_RESULT_ROW_FONT_SIZE,
          color: row.amountColor || HAND_RESULT_NEUTRAL_COLOR,
          fontStyle: "bold",
          fontFamily: UI_FONT_STACK,
        })
        .setDepth(HAND_RESULT_TEXT_DEPTH + 0.1)
      ...
      const rankText = this.add
        .text(CENTER_X + HAND_RESULT_RANK_X_OFFSET, detailY, row.statusText, {
          fontSize: HAND_RESULT_ROW_FONT_SIZE,
...
// Keep text anchored to avatar when switching hero/non-hero layout.
  ...
  renderState() {
    ...
    if (table) {
      ...
      if (Number.isFinite(currentHintHandId) && currentHintHandId !== this.lastHintHandId) {
  ...
  queueLeaveAction(type) {
    const isHandActive = this.state?.table?.status === "playing";
    if (isHandActive) {
      this.app.sendPacket("player_action", { action: "fold" });
    }
    if (type === "switch") {
      ...
      // Navigate to gameLobby now; leave_room deferred until hand ends
      const currentTableId = String(this.store.getState?.()?.table?.table_id ?? "");
