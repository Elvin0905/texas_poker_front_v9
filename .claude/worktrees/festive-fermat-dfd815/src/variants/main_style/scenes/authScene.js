...
export class AuthScene extends Phaser.Scene {
  ...
  create() {
    ...
    this._syncBound = () => {
      ...
      if (a === this._emailEl || a === this._pwEl) {
    ...
    this._onInputBlur = () => {
      ...
      this._kbTimer = setTimeout(() => {
        const a = document.activeElement;
        if (a !== this._emailEl && a !== this._pwEl) this._adjustForKeyboard(false);
      }, 200);
    };

    // Drag overlay: sits above canvas (z:50), below HTML inputs (z:100).
    ...
    Object.assign(this._kbOverlay.style, {
      ...
      zIndex: '50', display: 'none', touchAction: 'none',
    });
  ...
  _buildScene() {
    ...
    this.eyeIcon = this.add
      ...
      .setInteractive({
        hitArea: new Phaser.Geom.Rectangle(-39.5, -20, 79, 40),
        hitAreaCallback: Phaser.Geom.Rectangle.Contains,
        useHandCursor: true,
      });
    this.eyeIcon.on("pointerup", () => this._togglePasswordVisible());
  ...
  applyLayout() {
    ...
    this.forgotText?.setPosition(cx + 268, cy + 38);
  ...
  _syncInputPositions() {
    ...
    [this._emailEl, this._pwEl].forEach(el => el.style.removeProperty("display"));
    ...
    if (this._forgotHitEl) this._posEl(this._forgotHitEl, layout.centerX + 140, layout.centerY + 14, 142, 50, scale, ox, oy);
  }
  ...
  _hideForgotModal() {
    ...
    [this._fpOverlay, this._fpBorder, this._fpPanel, this._fpTitleLabel, this._fpTitle, this._fpDescText,
     this._fpBox1Gfx, this._fpMailIcon, this._fpBox2Gfx, this._fpCodeIcon,
     this._fpBox3Gfx, this._fpBox4Gfx, this._fpLock1Icon, this._fpLock2Icon,
     this._fpPrimaryBtn, this._fpCancelBtn]
      .forEach(o => o?.setVisible(false));
    this._syncInputPositions();
  }
