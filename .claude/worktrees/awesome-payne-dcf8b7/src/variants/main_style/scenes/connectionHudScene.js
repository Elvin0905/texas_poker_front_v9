...
const BADGE_OFFSET_RIGHT = 90;
const BADGE_OFFSET_BOTTOM = 30;
const BADGE_WIDTH = 94;
const BADGE_HEIGHT = 38;

const DETAIL_OFFSET_RIGHT = 90;
const DETAIL_OFFSET_BOTTOM = 151;
...
export class ConnectionHudScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.badgeBg = this.add
      .rectangle(0, 0, BADGE_WIDTH, BADGE_HEIGHT, 0x09131f, 0.78)
    ...
    this.barRects = heights.map((height, index) => {
      const bar = this.add.rectangle(-27 + index * 7, 7 - height / 2, 4, height, 0xffffff, 0.28).setOrigin(0.5);
    ...
    this.badgeHit = this.add
      .rectangle(0, 0, BADGE_WIDTH, BADGE_HEIGHT, 0x000000, 0.001)
    ...
    const detailBg = this.add
      .rectangle(0, 0, DETAIL_WIDTH, DETAIL_HEIGHT, 0x08121e, 0.94)
      .setStrokeStyle(1, 0xffffff, 0.2)
      .setOrigin(0.5);
    this.detailContainer.add(detailBg);

    this.detailHit = this.add
      .rectangle(0, 0, DETAIL_WIDTH, DETAIL_HEIGHT, 0x000000, 0.001)
    ...
    this.unsubscribeLayout = onLayoutResize(this, () => this.positionHud());
  ...
  positionHud() {
    const badgeX = layout.right - BADGE_OFFSET_RIGHT;
    const badgeY = layout.bottom - BADGE_OFFSET_BOTTOM - layout.safeAreaBottom;
    const detailX = layout.right - DETAIL_OFFSET_RIGHT;
    const detailY = layout.bottom - DETAIL_OFFSET_BOTTOM - layout.safeAreaBottom;

    this.badgeContainer?.setPosition(badgeX, badgeY);
    this.badgeHit?.setPosition(badgeX, badgeY);
    this.detailContainer?.setPosition(detailX, detailY);
    this.detailHit?.setPosition(detailX, detailY);
  }
